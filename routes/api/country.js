...
router.post(
  ...
  async (req, res) => {
    ...
    const { description_Fr, code } = req.body;
...
router.get('/', async (req, res) => {
  try {
    const elements = await Country.find().populate('owner', ['name']);
    res.json(elements);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
...
router.delete('/:type_id', auth, async (req, res) => {
  try {
    //remove type
    await Country.findOneAndRemove({ _id: req.params.type_id });
